...
#
#  $ ./run.sh [--work-dir /tmp/${USER}/fltest-feature-flsimskip0hits$] [--visu]
#
...
label="run-feature-flsimskip0hits"
with_visu=0
work_dir=""
cfg_dir=""
step1=1
...
echo >&2 "[info] skipNoHits = '${skipNoHits}'"
echo >&2 "[info] skipNoHitsOptions = '${skipNoHitsOptions}'"
...
if [ $step1 -eq 1 ]; then
    ...
    ========================================================
    STEP 1 - Realistic flat DBD source - 100 keV electrons
    ========================================================
EOF
    ...
    flsimulate-configure --no-gui \
			 ...
			 -s "primary_events:generator/if_aegir/selected=electron.200keV" \
			 -s "simulation:output_profile=all_details" \
			 -o "${FLWORKDIR}/simu.vprofile"
    if [ $? -ne 0 ]; then
    ...
    flsimulate ${skipNoHitsOptions} -c ${cfg_dir}/simu.conf -o ${FLWORKDIR}/feature-flsimskip0hits-1.brio # > ${FLWORKDIR}/flsim.log 2>&1
    if [ $? -ne 0 ]; then
	cat ${FLWORKDIR}/flsim.log
	...
	flvisualize \
	    ...
	    --input-file ${FLWORKDIR}/feature-flsimskip0hits-1.brio \
	    --focus-on-roi \
